...
export async function transformImages(projectId: string): Promise<string> {
  ...
  for (let i = 0; i < project.clips.length; i++) {
    ...
    try {
      ...
      console.log(`[transform] clip ${i} submitted: ${result.request_id}`);
    } catch (err: any) {
  ...
  for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt++) {
    ...
    for (let i = 0; i < project.clips.length; i++) {
      ...
      try {
        ...
        if (jobStatus === "COMPLETED") {
          ...
          console.log(`[transform] clip ${i} FAILED`);
        } else {
    ...
    project.progress = 10 + Math.round((done / project.clips.length) * 20);
    project.markModified("clips");
    await project.save();
