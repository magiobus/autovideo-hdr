...
export const generateVideosStep = createStep({
  id: "generate-videos",
  ...stepIO,
  execute: async ({ inputData }) => {
    ...
    for (let i = 0; i < project.clips.length; i++) {
      ...
      try {
        ...
        if (!result?.request_id) {
          throw new Error(`Fal submit returned no request_id: ${JSON.stringify(result)}`);
        }
    ...
    for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt++) {
      ...
      for (let i = 0; i < project.clips.length; i++) {
        ...
        try {
          ...
          if (jobStatus === "COMPLETED") {
            ...
            console.log(`[videogen] clip ${i} FAILED (fal status: FAILED)`);
          } else {
      ...
      project.progress = 30 + Math.round((done / totalClips) * 55);
      project.markModified("clips");
      await project.save();
